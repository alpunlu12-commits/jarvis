...
class JarvisUI:
    ...
    def _build_settings_panel(self):
        ...
        self._settings_tab_stt = tk.Canvas(
            self._settings_panel,
            width=80,
            height=28,
            bg="#041111",
        ...
        self._settings_tab_stt.bind("<Button-1>", lambda e: self._set_settings_tab("stt"))
        self._settings_body = tk.Frame(self._settings_panel, bg="#041111")
        self._debug_body = tk.Frame(self._settings_panel, bg="#041111")
        self._stt_body = tk.Frame(self._settings_panel, bg="#041111")

        # Build STT Settings Widgets
        self._build_stt_settings()
    ...
    def _draw_settings_tabs(self):
        for key, canvas, label in (
            ...
            ("stt", self._settings_tab_stt, "STT"),
        ):
    ...
    def _build_stt_settings(self):
        # We need to create labels, entries, and an update button for STT variables

        cfg = load_app_config()
        self._stt_energy_var = tk.StringVar(value=str(cfg.get("stt_energy_threshold", 4000)))
        self._stt_pause_var = tk.StringVar(value=str(cfg.get("stt_pause_threshold", 2.0)))
        self._stt_phrase_var = tk.StringVar(value=str(cfg.get("stt_phrase_threshold", 0.3)))
        self._stt_non_speaking_var = tk.StringVar(value=str(cfg.get("stt_non_speaking_duration", 0.5)))

        # Descriptions
        desc = [
            ("Gürültü Eşiği (Energy Threshold)", self._stt_energy_var, "Ortam gürültüsünü filtreler. Düşük: Hassas. Yüksek: Sadece yüksek ses (Örn: 4000)"),
            ("Bekleme Süresi (Pause Threshold)", self._stt_pause_var, "Sözünüzü bitirdiğinizde sistemin bekleyeceği saniye (Örn: 2.0)"),
            ("Minimum Kelime (Phrase Threshold)", self._stt_phrase_var, "Çok kısa seslerin (öksürük vb.) kelime sanılmasını önler (Örn: 0.3)"),
            ("Sessizlik Payı (Non-Speaking Dur)", self._stt_non_speaking_var, "Cümle sonlarındaki doğal sessizliği korur (Örn: 0.5)")
        ]

        y_offset = 10
        for title, var, description in desc:
            lbl_title = tk.Label(self._stt_body, text=title, fg=C_PRI, bg="#041111", font=font_body_bold(10))
            lbl_title.place(x=0, y=y_offset)
            y_offset += 22

            entry = tk.Entry(self._stt_body, textvariable=var, bg="#020a0a", fg=C_TEXT, font=font_body(10), insertbackground=C_TEXT)
            entry.place(x=0, y=y_offset, width=80, height=24)

            lbl_desc = tk.Label(self._stt_body, text=description, fg=C_DIM, bg="#041111", font=font_body(8), justify="left", wraplength=200)
            lbl_desc.place(x=90, y=y_offset)

            y_offset += 40

        btn = tk.Button(self._stt_body, text="KAYDET & UYGULA", bg=C_BLUE, fg="#ffffff", font=font_body_bold(9), command=self._save_stt_settings)
        btn.place(x=0, y=y_offset+10, width=150, height=30)

    def _save_stt_settings(self):
        try:
            energy = int(self._stt_energy_var.get())
            pause = float(self._stt_pause_var.get())
            phrase = float(self._stt_phrase_var.get())
            non_speaking = float(self._stt_non_speaking_var.get())

            if non_speaking > pause:
                self.write_log("SYS: Hata! Sessizlik Payı, Bekleme Süresi'nden büyük olamaz.")
                non_speaking = pause
                self._stt_non_speaking_var.set(str(non_speaking))

            save_app_config({
                "stt_energy_threshold": energy,
                "stt_pause_threshold": pause,
                "stt_phrase_threshold": phrase,
                "stt_non_speaking_duration": non_speaking
            })
            self.write_log("SYS: STT ayarları kaydedildi ve uygulandı.")
        except ValueError:
            self.write_log("SYS: STT Ayarları hatası! Lütfen sadece sayı giriniz.")
    ...
    def _place_layout_widgets(self):
        ...
        panel_y = geo["panel_y"]
        panel_w = geo["panel_w"]
        if self._settings_tab == "stt":
            panel_h = 420
        elif self._settings_tab == "debug":
            panel_h = 350
        else:
            panel_h = 292
        if self._settings_open:
            ...
            self._settings_tab_stt.place(x=234, y=40)
            if self._settings_tab == "debug":
                self._settings_body.place_forget()
                self._stt_body.place_forget()
            ...
            elif self._settings_tab == "stt":
                self._settings_body.place_forget()
                self._debug_body.place_forget()
                self._stt_body.place(x=12, y=76, width=panel_w - 24, height=panel_h - 88)
                self._stt_body.lift()
            else:
                self._debug_body.place_forget()
                self._stt_body.place_forget()
    ...
    def _show_setup_ui(self, edit_mode: bool = False):
        ...
        setup_h = min(640, max(550, int(self.H * 0.55)))
        self.setup_frame.place(relx=0.5, rely=0.5, anchor="center", width=setup_w, height=setup_h)
        self.setup_frame.pack_propagate(False)

        title = "◈ AYARLAR" if edit_mode else "◈ İLK KURULUM GEREKLİ"
        subtitle = "Yapay zeka sağlayıcınızı ve diğer ayarlarınızı yapılandırın."
        config = load_app_config()

        tk.Label(self.setup_frame, text=title,
                 fg=C_PRI, bg="#00080d", font=font_display(20)).pack(pady=(20, 6))
        tk.Label(self.setup_frame, text=subtitle,
                 fg=C_MID, bg="#00080d", font=font_body(11)).pack(pady=(0, 10))

        tk.Label(self.setup_frame, text="AI PROVIDER", fg=C_DIM, bg="#00080d", font=font_body(11)).pack(pady=(5, 2))
        self.provider_var = tk.StringVar(value=str(config.get("ai_provider", "gemini") or "gemini"))

        self.dynamic_frame = tk.Frame(self.setup_frame, bg="#00080d")
        ...
        def _on_provider_change(*args):
            ...
            if provider == "gemini":
                tk.Label(self.dynamic_frame, text="GEMINI API KEY", fg=C_DIM, bg="#00080d", font=font_body(11)).pack(pady=(5, 2))
                self.api_entry = tk.Entry(self.dynamic_frame, width=50, fg=C_TEXT, bg="#000d12", insertbackground=C_TEXT, borderwidth=0, font=font_body(12), show="*")
                self.api_entry.pack(pady=(0, 5), ipady=5)
                current_key = str(config.get("gemini_api_key", "") or "")
                if current_key:
                    self.api_entry.insert(0, current_key)
            elif provider == "ollama":
                tk.Label(self.dynamic_frame, text="OLLAMA HOST (örn. http://127.0.0.1:11434)", fg=C_DIM, bg="#00080d", font=font_body(11)).pack(pady=(5, 2))

                host_frame = tk.Frame(self.dynamic_frame, bg="#00080d")
                host_frame.pack(pady=(0, 5))

                self.ollama_host_entry = tk.Entry(host_frame, width=38, fg=C_TEXT, bg="#000d12", insertbackground=C_TEXT, borderwidth=0, font=font_body(12))
                self.ollama_host_entry.pack(side="left", ipady=5, padx=(0, 5))
                current_host = str(config.get("ollama_host", "http://127.0.0.1:11434") or "http://127.0.0.1:11434")
                self.ollama_host_entry.insert(0, current_host)

                tk.Label(self.dynamic_frame, text="OLLAMA MODEL", fg=C_DIM, bg="#00080d", font=font_body(11)).pack(pady=(5, 2))

                self.ollama_model_var = tk.StringVar()
                self.ollama_model_menu = tk.OptionMenu(self.dynamic_frame, self.ollama_model_var, "Yükleniyor...")
                self.ollama_model_menu.config(width=45, fg=C_TEXT, bg="#000d12", highlightthickness=0, font=font_body(11))
                self.ollama_model_menu.pack(pady=(0, 5))

                def fetch_models(*args):
                    self.ollama_model_var.set("Yükleniyor...")
                    host_val = self.ollama_host_entry.get().strip() or "http://127.0.0.1:11434"
                    def _do_fetch(host):
                        models = []
                        try:
                        ...
                        if not models:
                            models = [current_model] if current_model else ["llama3", "mistral", "gemma"]

                        def update_menu():
                            menu = self.ollama_model_menu["menu"]
                            menu.delete(0, "end")
                            for m in models:
                                menu.add_command(label=m, command=tk._setit(self.ollama_model_var, m))
                    ...
                    import threading
                    threading.Thread(target=_do_fetch, args=(host_val,), daemon=True).start()

                tk.Button(host_frame, text="🔄", command=fetch_models, bg=C_BG, fg=C_PRI, borderwidth=0, font=font_body_bold(10)).pack(side="left")

                fetch_models()


        self.provider_menu = tk.OptionMenu(self.setup_frame, self.provider_var, "gemini", "ollama", command=_on_provider_change)
        self.provider_menu.config(width=45, fg=C_PRI, bg="#000d12", highlightthickness=0, font=font_body_bold(11))
        self.provider_menu.pack(pady=(0, 5))
        ...
        tk.Label(self.setup_frame, text="YOUTUBE API KEY",
                 fg=C_DIM, bg="#00080d", font=font_body(11)).pack(pady=(5, 2))
        ...
        self.youtube_api_entry.pack(pady=(0, 5), ipady=5)
        ...
        self.youtube_handle_entry.pack(pady=(0, 5), ipady=5)
        ...
        tk.Button(buttons, text="▸ KAYDET",
                  ...
                  borderwidth=0, padx=24, pady=8).pack(side="left", padx=8)
    ...
    def _save_api_key(self):
        ...
        ollama_model_val = "llama3"

        if getattr(self, "api_entry", None) is not None and self.api_entry.winfo_exists():
            key_val = self.api_entry.get().strip()

        if getattr(self, "ollama_host_entry", None) is not None and self.ollama_host_entry.winfo_exists():
            ollama_host_val = self.ollama_host_entry.get().strip()

        if hasattr(self, "ollama_model_var"):
            val = self.ollama_model_var.get().strip()
            if val and val != "Yükleniyor...":
                ollama_model_val = val

        if not hasattr(self, "ollama_model_var") or self.ollama_model_var.get().strip() in ["", "Yükleniyor..."]:
            from app_config import load_app_config
            cfg = load_app_config()
            ollama_model_val = str(cfg.get("ollama_model", "llama3") or "llama3")

        if provider == "gemini" and not key_val:
            return

        youtube_key = self.youtube_api_entry.get().strip() if getattr(self, "youtube_api_entry", None) else ""
        youtube_handle = self.youtube_handle_entry.get().strip() if getattr(self, "youtube_handle_entry", None) else ""
        ...
        save_app_config(
            {
                ...
                "voice": getattr(self, "_current_voice", "Charon"),
            }
        )
        self._close_setup_ui()
