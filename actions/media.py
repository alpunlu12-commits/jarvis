...
if sys.platform == "darwin":
    ...
    SPOTIFY_APP = os.path.expandvars(r"%APPDATA%\Spotify\Spotify.exe")
else:
    SPOTIFY_APP = "/usr/bin/spotify" # Common linux path, fallback to cmd
...
def _play_spotify(query: str, autoplay: bool = True) -> str:
    ...
    if sys.platform == "darwin":
        ...
        return f"Spotify araması açıldı ama otomatik oynatma tamamlanamadı: {detail}. Erişilebilirlik izni gerekebilir."
    else:
        time.sleep(2)
        pyautogui.press('tab')
        time.sleep(0.2)
...
def _play_music_app(query: str, autoplay: bool = True) -> str:
    if sys.platform != "darwin":
        return "Apple Music sadece macOS'ta destekleniyor."
