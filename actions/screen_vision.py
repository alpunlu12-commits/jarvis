...
def _run_helper(mode: str, timeout: int = 20) -> tuple[bool, str]:
    if sys.platform == "darwin":
        import shutil
        helper_path = shutil.which("jarvis-screen-helper")
        if not helper_path:
            return False, "macOS ekran okuyucu helper'i bulunamadi."
        ...
        try:
            ...
            return False, "Ekran helper'i zaman asimina ugradi."
        except Exception as exc:
            return False, f"Ekran helper'i calistirilamadi: {exc}"
    else:
        # Cross platform fallback using PyAutoGUI / ImageGrab
        try:
            temp_dir = tempfile.gettempdir()
            filename = f"jarvis_screen_{uuid.uuid4().hex[:8]}.png"
            ...
            if bounds and sys.platform == "win32":
                # If we have valid bounds, grab only that region
                img = ImageGrab.grab(bbox=(bounds["x"], bounds["y"], bounds["x"] + bounds["width"], bounds["y"] + bounds["height"]))
            else:
                # Fallback to full screen if bounding box fails or we're on linux where pygetwindow might struggle
                img = ImageGrab.grab()
...
def _vision_prompt(query: str, owner_name: str, window_title: str) -> str:
    ...
    return (
        "Sen JARVIS icin ekran analizi yapan bir goruntu yorumlayicisisin.\n"
